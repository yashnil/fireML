...
def generate_veg_elev_tables_visual(
    dataset_path="/Users/yashnilmohanty/Desktop/final_dataset.nc",
    veg_var="VegTyp",
    elev_var="Elevation",
    ...
    elev_bins=None,
    pixel_area_km2=1.0,
    csv_fraction="veg_elev_fraction.csv",
    html_fraction="veg_elev_fraction.html",
    csv_counts="veg_elev_counts.csv",
    html_counts="veg_elev_counts.html"
):
    """
    Generates TWO VegType vs. Elevation tables for burned pixels:
      1) A fraction table => each cell is n/N
         (n = # burned pixels in that VegType/ElevBin, N = total burned).
      2) A counts table => each cell is just n (# burned pixels).
    
    Both tables are saved as CSV & HTML for clarity and downloaded usage.
    ...
    if "year" in veg_data.dims:
        veg_types = veg_data.isel(year=0).values  # shape: (pixel,)
    else:
    ...
    if "year" in elev_data.dims:
        elevations = elev_data.isel(year=0).values  # shape: (pixel,)
    else:
    ...
    # Compute cumulative burn fraction => shape (pixel,)
    burn_sum = np.nansum(burn_2d, axis=0)

    # Classify burned pixels
    ...
    # Mean burn fraction among burned pixels
    mean_burn_fraction = np.nanmean(burn_sum[burned_mask])

    # Define elevation bins if not provided
    if elev_bins is None:
        elev_bins = np.arange(0, 5000, 500)  # e.g. 0-500, 500-1000, ...
    ...
    # We'll style the counts table with a color gradient
    # (You can choose a different colormap if you prefer)
    styled_counts = counts_table.style.background_gradient(cmap="Blues")
    styled_counts.set_caption("VegType × Elevation Burn - Counts Table")
    ...
    # We'll style the fraction table with a color gradient
    styled_fraction = fraction_table.style.background_gradient(cmap="viridis")
    styled_fraction.set_caption("VegType × Elevation Burn - Fraction Table")
...
if __name__ == "__main__":
    generate_veg_elev_tables_visual(
        dataset_path="/Users/yashnilmohanty/Desktop/final_dataset.nc",
        veg_var="VegTyp",
        elev_var="Elevation",
        burn_var="burn_fraction",
