...
# ────────────────────────────────────────────────────────────
#  plotting helpers  (all legends preserved)
# ────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────
#  main RF experiment (70/30 per category)
# ────────────────────────────────────────────────────────────
def rf_experiment(X,y,cat2d,ok,ds,feat_names):
    ...
    # 1) train / test plots
    plot_scatter(Ytr,rf.predict(Xtr),"Train (70 %)")
    plot_bias_hist(Ytr,rf.predict(Xtr),"Bias Hist: Train")
    Yhat_te=rf.predict(Xte)
    plot_scatter(Yte,Yhat_te,"Test (30 %)")
    plot_bias_hist(Yte,Yhat_te,"Bias Hist: Test")
    ...
    for c in (0,1,2,3):
        m=cat_te==c
        if m.any():
            plot_scatter(Yte[m],Yhat_te[m],f"Test cat={c}")
            plot_bias_hist(Yte[m],Yhat_te[m],f"Bias Hist: cat={c}")

    # 3) all‑data colour‑coded
    plot_scatter_by_cat(Yte,Yhat_te,cat_te,"All Test Data (colour by cat)")
    ...
    plot_top5_feature_scatter(rf,Xte,Yte,cat_te,feat_names,"Top‑5")
    ...
    if "burn_fraction" in feat_names:
        ...
        eval_bins(Yte,Yhat_te,Xte[:,bf_idx],bins)
...
if __name__=="__main__":
    ...
    # cumulative‑burn categories already stored in the file
    # (if not, load / compute as in earlier scripts)
    cat_2d=ds["burn_cumsum"].copy()
    cat_2d.values[:]=0
    cat_2d.values[ds["burn_cumsum"]<0.25]=0
    cat_2d.values[(ds["burn_cumsum"]>=0.25)&(ds["burn_cumsum"]<0.5)]=1
    cat_2d.values[(ds["burn_cumsum"]>=0.5)&(ds["burn_cumsum"]<0.75)]=2
    cat_2d.values[ds["burn_cumsum"]>=0.75]=3
    cat_2d=cat_2d.values.astype(int)
