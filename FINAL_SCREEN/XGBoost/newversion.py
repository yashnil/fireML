#!/usr/bin/env python3
# ============================================================
#  Fire-ML · XGBoost experiment on final_dataset5.nc
...
def plot_scatter(y_true, y_pred, title=None):
    print(f"\n[XGBoost] Rendering scatter plot: Predicted vs Observed DSD")
...
def plot_scatter_by_cat(y_true, y_pred, cat, title=None):
    print(f"\n[XGBoost] Rendering scatter plot: All categories colored (red=c0, yellow=c1, green=c2, blue=c3)")
    fig, ax = plt.subplots(figsize=(6,6))
...
def plot_scatter_density_by_cat(y_true, y_pred, cat, cat_idx,
    ...
    """
    print(f"[XGBoost] Rendering density scatter plot: Category {cat_idx} with color-coded point density")
    sel = (cat == cat_idx)
...
def plot_bias_hist(y_true, y_pred, title=None,
    ...
    print(f"[XGBoost] Rendering bias histogram: Distribution of prediction errors")
    res = y_pred - y_true
...
def bias_map_ca(ds, pix_idx, y_true, y_pred):
    ...
    print("[XGBoost] Rendering spatial bias map: California with pixel-level prediction bias (±30 days)")
    merc = ccrs.epsg(3857)
...
def boxplot_dod_by_elev_veg(y, elev, veg):
    print("[XGBoost] Rendering boxplot: DSD distribution by elevation bins and vegetation types")
...
def heat_bias_by_elev_veg(y_true, y_pred, elev, veg,
                          elev_edges=(500,1000,1500,2000,2500,
                                      3000,3500,4000,4500)):
    print("[XGBoost] Rendering heatmap: Mean bias by elevation bins and vegetation types")
    bias     = y_pred - y_true
...
# ─── XGBOOST EXPERIMENT ────────────────────────────────────────────
def xgboost_unburned_experiment(X, y, cat2d, ok, ds, feat_names,
                                 unburned_max_cat=0,
    ...
    # helper to invert (not needed for XGBoost, but kept for consistency)
    def inv(a): return a

    # A) TRAIN / TEST
    print(f"\n{'='*60}")
    print(f"[XGBoost] Starting plotting sequence for unburned_max_cat={thr}")
    print(f"[XGBoost] Training on categories ≤ {thr}, evaluating on all categories")
    print(f"{'='*60}")
    yhat_tr = inv(xgb.predict(X_tr))
    plot_scatter(y_tr, yhat_tr, f"XGBoost TRAIN (cat ≤ {thr})")
    plot_bias_hist(y_tr, yhat_tr)

    yhat_te = inv(xgb.predict(X_te))
    plot_scatter(y_te, yhat_te, f"XGBoost TEST  (cat ≤ {thr})")
    plot_bias_hist(y_te, yhat_te)

    # B) ALL-DATA
    yhat_all = inv(xgb.predict(Xv))
...
if __name__=="__main__":
    ...
    log("XGBoost RUN #1: unburned cat 0")
    xgboost_unburned_experiment(X_all, y_all, cat2d, ok, ds, feat_names,
                                 unburned_max_cat=0, n_estimators=100, max_depth=6)
    log("XGBoost RUN #2: unburned cat 0+1")
    xgboost_unburned_experiment(X_all, y_all, cat2d, ok, ds, feat_names,
                                 unburned_max_cat=1, n_estimators=100, max_depth=6)
    log("ALL DONE.")
