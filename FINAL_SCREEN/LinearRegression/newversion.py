#!/usr/bin/env python3
# ============================================================
#  Fire-ML · Linear Regression experiment on final_dataset5.nc
...
def plot_scatter(y_true, y_pred, title=None):
    print(f"\n[Linear Regression] Rendering scatter plot: Predicted vs Observed DSD")
...
def plot_scatter_by_cat(y_true, y_pred, cat, title=None):
    print(f"\n[Linear Regression] Rendering scatter plot: All categories colored (red=c0, yellow=c1, green=c2, blue=c3)")
    fig, ax = plt.subplots(figsize=(6,6))
...
def plot_scatter_density_by_cat(y_true, y_pred, cat, cat_idx,
    ...
    """
    print(f"[Linear Regression] Rendering density scatter plot: Category {cat_idx} with color-coded point density")
    sel = (cat == cat_idx)
...
def plot_bias_hist(y_true, y_pred, title=None,
    ...
    print(f"[Linear Regression] Rendering bias histogram: Distribution of prediction errors")
    res = y_pred - y_true
...
def bias_map_ca(ds, pix_idx, y_true, y_pred):
    ...
    print("[Linear Regression] Rendering spatial bias map: California with pixel-level prediction bias (±30 days)")
    merc = ccrs.epsg(3857)
...
def boxplot_dod_by_elev_veg(y, elev, veg):
    print("[Linear Regression] Rendering boxplot: DSD distribution by elevation bins and vegetation types")
...
def heat_bias_by_elev_veg(y_true, y_pred, elev, veg,
                          elev_edges=(500,1000,1500,2000,2500,
                                      3000,3500,4000,4500)):
    print("[Linear Regression] Rendering heatmap: Mean bias by elevation bins and vegetation types")
    bias     = y_pred - y_true
...
def linear_unburned_experiment(X, y, cat2d, ok, ds, feat_names,
                                unburned_max_cat=0):
...
if __name__=="__main__":
    ...
    log("Linear Regression RUN #1: unburned cat 0")
    linear_unburned_experiment(X_all, y_all, cat2d, ok, ds, feat_names,
                               unburned_max_cat=0)
    log("Linear Regression RUN #2: unburned cat 0+1")
    linear_unburned_experiment(X_all, y_all, cat2d, ok, ds, feat_names,
                               unburned_max_cat=1)
    log("ALL DONE.")
