#!/usr/bin/env python3
# ============================================================
#  Fire-ML · LSTM evaluation on final_dataset5.nc  (Experiment 3)
#  70 %/30 % split inside each burn-category
#  burn_fraction INCLUDED · all *aorcSummer* predictors EXCLUDED
...
def gather_features_incbf(ds, target="DSD"):
    ...
    Keep every data variable except
      • the target itself
      • simple coords / bookkeeping arrays
      • ANY variable whose name begins with 'aorcSummer'
    (burn_fraction is retained)
    """
    ...
    for v in ds.data_vars:
        if v.lower() in base_skip or v.lower().startswith("aorcsummer"):
            continue
...
if __name__ == "__main__":
    ...
    log("ALL DONE (Experiment 3 – LSTM)")
